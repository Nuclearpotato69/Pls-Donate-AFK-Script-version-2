...
local win = Flux:Window("CapySploit", "by punchy#0411", Color3.fromRGB(0, 128, 0), Enum.KeyCode.RightShift)
...
otherTab:Button("Discord Server", "Copies Discord server link", function()
    ...
    if cboard then
        cboard("https://discord.gg/Kdpuu8Xx6T")
    end
    Flux:Notification("Copied to clipboard", "https://discord.gg/Kdpuu8Xx6T", "Okay")
end)
    ...
    if getgenv().settings.webhookToggle and getgenv().settings.webhookBox then
        task.spawn(function()
            ...
            if not sent then
                ...
                webhook(tostring("💰 Somebody tipped "..amount.." Robux to "..Players.LocalPlayer.DisplayName.." *[After Tax: "..math.floor(amount * 0.7).."]* (Total: " ..newRaised..")"))
            end
            RaisedC = newRaised
